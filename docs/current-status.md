...
最后更新：2026-06-09（E.3.5 Agent 运行态完整体验已交付）
...
**当前开发重点（后端优先）：** 见 [【E 节】](#e-后端优先路线当前采用) —— **P3 E.4**（新工具 + 安全治理）。E.3.5 RunTimeline 体验已交付。
...
**你现在最该做（P3）：** **E.4 新工具 + 安全治理** —— 在 RunTimeline 上复用同一套 SSE 事件，扩展 eval 覆盖。

**E.3.5 已完成：** 真 LLM streaming、`planner_decision`、RunTimeline、Markdown、步骤动画（见 [E.3.5](#e35-agent-运行态完整体验cursor-式)）。
...
P2  E.3 SSE 通路（最小联调）          ← 已完成
P2.5 E.3.5 Agent 运行态完整体验      ← 已完成
P3   E.4 新工具 + 安全治理            ← 当前优先
```
