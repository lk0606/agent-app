...
|--|--|
| **状态** | 已完成 |
| **目标** | 扩 eval 覆盖多轮记忆、SSRF、read_file 沙箱、改坏实验；强化断言与失败时 DB 还原 |
| **改动范围** | `apps/api/evals/cases/`、`apps/api/src/scripts/run-evals.ts`、`docs/backend-learning/eval-break-lab.md` |

**已交付：**

- 用例 11 → **18 条**（`basic-agent-cases.json`）
- 新增：`session-memory-name`（3 轮记名）、`session-then-time-tool`（记忆 + 工具）、`blocked-http-10-network` / `blocked-http-192-network`、`blocked-read-hidden-dotenv` / `blocked-read-bad-extension`、`read-file-no-secret-leak`
- 既有安全用例补 `expectedTools`；`maxToolCalls` 统一为**仅计成功**的工具次数
- `run-evals.ts`：失败时从 `tool_calls` 表还原尝试记录；新增 `forbiddenKeywords` 断言
- 改坏实验手册：**[`docs/backend-learning/eval-break-lab.md`](backend-learning/eval-break-lab.md)**（5 组故意改坏 + 预期 fail）
...
- 预期：`passed: 18`、`failed: 0`；报告 `apps/api/evals/reports/eval-run-*.json`
- 改坏实验：按 `eval-break-lab.md` 改 prompt / 工具 / 上下文 → 重跑 → 确认目标 case fail → 还原 → 全绿
- 某条失败：`taskId` → `pnpm run task:replay -- <taskId>`，对照 `failures[]` 看 `plannerTrace` / `tool_calls`

**学习要点：**

1. **eval 是改代码前的安全网**：扩 case 比「感觉没问题」可靠；改坏实验验证网确实能报警。
2. **安全断言要看「工具尝试」**：失败任务 `PlannerAgent` 会 throw，断言需从 DB 读 `tool_calls`（含 `status=failed`）。
3. **`maxToolCalls` 只限成功次数**：SSRF / 沙箱拦截允许「调了工具但失败」，`maxToolCalls: 0` 仍可通过。
4. **`forbiddenKeywords`**：防模型在回答里复述敏感提示语（如 fixture 里的提醒句），与 Tool 层 enforce 互补。
5. **多轮 `steps[]`**：可测 3 轮记名、第 2 轮要工具等组合场景，断言仍只看最后一轮。

**代码怎么读：**

| 顺序 | 文件 | 看什么 |
|------|------|--------|
| 1 | `apps/api/evals/cases/basic-agent-cases.json` | 18 条分类：工具命中 / 记忆 / 安全 / 禁词 |
| 2 | `apps/api/src/scripts/run-evals.ts` | `loadToolCallsFromDb`、`forbiddenKeywords`、`maxToolCalls` 成功计数 |
| 3 | `docs/backend-learning/eval-break-lab.md` | 5 组改坏实验与预期 failures |
| 4 | `apps/api/src/memory/postgres-memory-store.ts` | `listTaskToolCalls` — eval 失败还原的数据源 |

心智模型：`case JSON` → `run-evals`（跑 TaskRunner + DB 断言）→ 改 prompt/工具前先跑全绿 → 改坏确认能 fail → 再开发 E.6-B/C。

---

### E.6-B 新工具 `list_dir` + 沙箱安全

| | |
|--|--|
| **状态** | 已完成 |
| **目标** | 练「加工具」全流程：实现 → 注册 → Planner prompt → eval 正常 + 安全 case |
| **改动范围** | `list-dir-tool.ts`、`env.ts`、`create-agent-runtime.ts`、`hunyuan-llm-client.ts`、eval cases |

#### 已交付

| 项 | 说明 |
|----|------|
| **`list_dir` 工具** | 列出 `READ_FILE_ROOT_DIR` 沙箱内文件/子目录；隐藏文件（`.` 开头）不展示 |
| **安全治理** | 与 `read_file` 共用沙箱根目录；禁止 `../`、绝对路径；`LIST_DIR_MAX_ENTRIES` 限制返回条数 |
| **Planner prompt** | 列举/浏览沙箱目录场景引导调用 `list_dir` |
| **eval 用例 +2** | `list-dir-fixture` / `blocked-list-dir-traversal`；既有 case 的 `forbiddenTools` 补 `list_dir` |
| **RunTimeline** | 无需改 UI，复用 `tool_start` / `tool_end` |
...
curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
  -d '{"input":"请用 list_dir 列出沙箱根目录有哪些文件"}' | jq .

curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
  -d '{"input":"请用 list_dir 列出 ../../../etc 目录"}' | jq .
# 预期：task failed，error.code = BAD_REQUEST

pnpm run evals:run
# 预期：20 条用例全过（含 2 条 list_dir 相关）
```
...
1. **新工具最小闭环**：`Tool` 接口 → `create-agent-runtime` 注册 → Planner system prompt 补选用场景 → eval 断言。
2. **沙箱复用**：`list_dir` 与 `read_file` 共用 `READ_FILE_ROOT_DIR`，路径前缀校验模式一致，不必另起一套根目录。
3. **列目录 vs 读文件**：列目录不校验扩展名白名单，但仍要防路径穿越与绝对路径；隐藏文件在列表层过滤，与 `read_file` denylist 互补。
4. **eval 分工**：`list-dir-fixture` 测工具命中 + 关键词；`blocked-list-dir-traversal` 用绝对路径 `/etc` 测 Tool 层 `BAD_REQUEST`（与 `blocked-read-absolute-path` 同模式，避免模型对 `../../../etc` 口头拒绝不调工具）。

#### 代码怎么读

| 顺序 | 文件 | 看什么 |
|------|------|--------|
| 1 | `apps/api/src/tools/list-dir-tool.ts` | `resolveSafePath`、隐藏文件过滤、`maxEntries` |
| 2 | `apps/api/src/config/env.ts` | `LIST_DIR_MAX_ENTRIES`（根目录复用 `READ_FILE_ROOT_DIR`） |
| 3 | `apps/api/src/app/create-agent-runtime.ts` | 工具注册顺序 |
| 4 | `apps/api/src/llm/hunyuan-llm-client.ts` | Planner system prompt 新增 list_dir 规则 |
| 5 | `apps/api/evals/cases/basic-agent-cases.json` | 2 条新 case + `forbiddenTools` 更新 |
| 6 | [`docs/backend-learning/list-dir-tool-notes.md`](backend-learning/list-dir-tool-notes.md) | `readdir` / `Dirent` / 一层 listing / `other` 类型 |

心智模型：用户要「看有哪些文件」→ Planner 选 `list_dir` → `readdir` 沙箱内 → 模型根据 listing 组织回答；越界路径在 `execute()` 内直接失败。
...
| | |
|--|--|
| **状态** | E.7-A 已完成；**E.7-B 已完成** |
| **目标** | 自己串 RAG 闭环：切块 → 索引 → 检索 Tool → Planner 选用 → eval；阶段 2 补 embedding 向量与 hybrid |
| **学习计划** | **[`docs/backend-learning/lightweight-rag-plan.md`](backend-learning/lightweight-rag-plan.md)**（分阶段、读码顺序、自检清单） |
| **改动范围** | `rag/document-index.ts`、`search-docs-tool.ts`、`env.ts`、`create-agent-runtime.ts`、eval cases |

#### E.7-B 已交付（向量 + hybrid）

| 项 | 说明 |
|----|------|
| **Embedding API** | `TokenHubEmbeddingClient`；默认模型 `kinfra-text-embedding-0.6b`；须 `encoding_format: float` |
| **持久化** | 表 `document_chunks`（JSONB embedding，不依赖 pgvector） |
| **离线索引** | `pnpm run rag:index`；eval 在 vector/hybrid 且表空时自动补索引 |
| **检索模式** | `SEARCH_DOCS_MODE`：`keyword`（默认）\| `vector` \| `hybrid` |
| **eval +1** | `search-docs-city-zh`（中文「台北」→ Taipei；仅 hybrid/vector 模式跑） |
| **基线** | keyword 模式 **22 条**；hybrid 模式 **23 条**（含同义检索） |

#### E.7-B 测试方法

**启动依赖：** `docker compose` + `db:migrate`；TokenHub Key；hybrid/vector 须先 `pnpm run rag:index`。

```bash
pnpm run check:all
pnpm run db:migrate

# 1. 建向量索引（首次或 fixture 变更后）
pnpm run rag:index
# 预期：chunkCount ≥ 1；embeddingModel=kinfra-text-embedding-0.6b

# 2. hybrid 手测 — 中文同义检索「台北」→ Taipei
SEARCH_DOCS_MODE=hybrid pnpm run dev:server
curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
  -d '{"input":"请用 search_docs 搜索文档里提到的台北，直接回答城市英文名"}' \
  | jq -r '.result.summary'
# 预期：summary 含 Taipei；toolCalls[0].output 含 Search mode: hybrid

# 3. keyword 回归（22 条，跳过 search-docs-city-zh）
pnpm run evals:run
jq '{total, passed, failed}' "$(ls -t apps/api/evals/reports/eval-run-*.json | head -1)"

# 4. hybrid 全量（23 条，含 search-docs-city-zh）
SEARCH_DOCS_MODE=hybrid pnpm run evals:run
jq '{total, passed, failed}' "$(ls -t apps/api/evals/reports/eval-run-*.json | head -1)"
```

- **失败排查：** embedding 400 → 检查 `HUNYUAN_EMBEDDING_MODEL` 是否为 TokenHub 向量模型；向量索引空 → 先 `rag:index`
- **改坏实验（可选）：** 清空 `document_chunks` 后 hybrid 检索应提示运行 `rag:index`

#### E.7-B 学习要点

1. **向量检索补关键词盲区**：「台北」与文档里 "Taipei" 无字面重叠，靠 embedding 余弦相似度召回。
2. **离线建索引 vs 在线查询**：`rag:index` 批量 embed 写 DB；`search_docs` 只对 query 调一次 embedding API。
3. **JSONB 存向量**：学习项目不必上 pgvector；应用层 `cosineSimilarity` 够用。
4. **hybrid 加权合并**：关键词 0.4 + 向量 0.6，兼顾精确词匹配与语义召回。
5. **TokenHub 陷阱**：embedding 模型用 `kinfra-text-embedding-0.6b`，且须显式 `encoding_format: "float"`。
6. **余弦野路子理解**（入参出参 / 分子分母 / 假数字手算）：[`docs/backend-learning/embedding-cosine-notes.md`](backend-learning/embedding-cosine-notes.md)

#### E.7-B 代码怎么读

| 顺序 | 文件 | 看什么 |
|------|------|--------|
| 1 | `apps/api/src/llm/embedding-client.ts` | OpenAI 兼容 `/embeddings` |
| 2 | `apps/api/infra/postgres/init/005_document_chunks.sql` | 向量持久化表 |
| 3 | `apps/api/src/rag/build-document-index.ts` | 切块 → 批量 embed → replaceAll |
| 4 | `apps/api/src/scripts/rag-index.ts` | CLI 入口 |
| 5 | `apps/api/src/rag/document-index.ts` | `searchVector` / `searchByMode` / hybrid merge |
| 6 | `apps/api/src/tools/search-docs-tool.ts` | 按模式懒加载关键词索引与向量块 |

心智模型：`rag:index` 离线 embed 写 `document_chunks` → 运行时 `search_docs` embed query → 余弦 top-k → 片段注入 tool output。

#### E.7-A 已交付（关键词检索）

| 项 | 说明 |
|----|------|
| **`DocumentIndex`** | 遍历沙箱 `.txt/.md` → 段落切块 → 内存索引 |
| **`search_docs` 工具** | 关键词打分 top-k；与 `read_file` 共用 `READ_FILE_ROOT_DIR` |
| **Planner prompt** | 跨文件搜索场景引导 `search_docs`；路径明确仍用 `read_file` |
| **fixture +1** | `travel-notes.md`（含 Osaka，供跨文件检索 case） |
| **eval 用例 +2** | `search-docs-city` / `search-docs-japan-city`；基线 **22 条** |

#### 测试方法

**启动依赖：** `docker compose` + `db:migrate`；`apps/api/.env` 配好 TokenHub 三件套；**新增工具后须重启 `dev:server`**。
...
# 1. 关键词检索 — favorite city → Taipei（推荐用 jq 抽字段，见 search-docs-tool-notes.md §Tool 输出格式）
curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
...
# 只看人话：... | jq -r '.result.summary'

# 2. 跨文件检索 — Japanese city → Osaka（阶段 1 须用英文 query 词，见 search-docs-tool-notes.md）
curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
  -d '{"input":"请用 search_docs 搜索文档中的 Japanese city，直接回答城市英文名"}' | jq .
# 预期：toolCalls[0].toolName = search_docs；summary 含 Osaka

# 3. 对照 DB（可选）
export TASK_ID=$(curl -s -X POST http://localhost:3000/agent/run \
  -H 'content-type: application/json' \
...
- **改坏实验（可选）：** [`docs/backend-learning/eval-break-lab.md`](backend-learning/eval-break-lab.md) §实验 6
- **完整测试方案：** [`docs/backend-learning/search-docs-tool-notes.md`](backend-learning/search-docs-tool-notes.md) §E.7-A 测试方案

#### 学习要点

1. **RAG 是 Tool，不是中间件**：检索结果走 `tool_calls`，eval 可断言 `expectedTools: ["search_docs"]`。
2. **三工具分工**：`list_dir` 列名、`read_file` 读已知路径、`search_docs` 跨文件搜片段。
3. **阶段 1 故意简单**：关键词匹配够用；同义词漏召留到 E.7-B 向量检索。
4. **沙箱复用**：索引扫描边界与 `read_file` 扩展名/禁读名单一致。
5. **自己串 vs 框架**：embedding API 阶段 2 再用；E.7-A 不上 LangChain/pgvector。

#### 代码怎么读

| 顺序 | 文件 | 看什么 |
|------|------|--------|
| 1 | [`docs/backend-learning/lightweight-rag-plan.md`](backend-learning/lightweight-rag-plan.md) | 阶段划分、自检清单 |
| 2 | `apps/api/src/rag/document-index.ts` | `build`、`chunkText`、`search` |
| 3 | `apps/api/src/tools/search-docs-tool.ts` | Tool 接口、懒加载索引 |
| 4 | `apps/api/src/app/create-agent-runtime.ts` | 注册与 read_file 共用配置 |
| 5 | [`docs/backend-learning/search-docs-tool-notes.md`](backend-learning/search-docs-tool-notes.md) | 切块、打分、改坏思路 |

心智模型：用户只有问题没有路径 → Planner 选 `search_docs` → 片段注入 Tool output → LLM 组织回答。
...
|--|--|
| **状态** | 已完成 |
| **目标** | 运行中任务可被取消或超时中止；`tasks.status=cancelled` 与业务 `failed` 区分 |
| **改动范围** | `RunningTaskRegistry`、`TaskRunner` AbortController、`PlannerAgent` 协作点、`POST /tasks/:id/cancel`、SSE 断开 abort、契约 `cancelled` 状态 |

#### 已交付

| 项 | 说明 |
|----|------|
| **AbortController 登记** | `running-task-registry.ts`：run 开始 register、结束 unregister |
| **协作取消** | `throwIfAborted` 在每步 plan 前/后、工具执行前/后检查 |
| **取消 API** | `POST /tasks/:taskId/cancel` → abort → 最终 status=`cancelled`、`errorCode=CANCELLED` |
| **可中断 wait** | `WaitTool`：切片 sleep + 检查 signal；手测/cancel smoke 用 |
| **SSE 断开** | `req.on('close')` 在未写完响应时 abort |
| **超时** | `AGENT_TASK_TIMEOUT_MS`（可选）；eval 可用 `taskTimeoutMs` 覆盖 |
| **契约** | `TaskStatusSchema` 增 `cancelled`；`CancelTaskResponseSchema` |
| **eval +1** | `task-timeout-smoke`（`taskTimeoutMs: 1` → `TIMEOUT_ERROR` + cancelled） |
| **smoke** | `pnpm run smoke:cancel`（等 wait 的 tool_start 再 cancel） |
...
# 1. 超时回归（不需起 server；与「手测取消」无关）
pnpm run evals:run -- --id task-timeout-smoke
# 预期：passed；taskStatus=cancelled；errorCode=TIMEOUT_ERROR

# 2. 推荐：自动手测取消（先起 server，再另开终端）
pnpm run dev:server          # 终端 1；改过工具后须重启
pnpm run smoke:cancel        # 终端 2
# 预期：出现 tool_start wait → cancel cancelled=true → status=cancelled / CANCELLED

# 3. 手动双终端（窗口够长：wait 15 秒；不必开前端）
# 终端 A：首帧 thinking 的 data.taskId 立刻出现，直接抄 UUID
curl -N -X POST http://localhost:3000/agent/stream \
  -H 'content-type: application/json' \
  -d '{"input":"请务必调用 wait 工具等待 15 秒，结束后只回复完成"}'
# 终端 B（换成真实 UUID，不要写字面量 <taskId>）：
curl -s -X POST http://localhost:3000/tasks/真实UUID/cancel | jq .
curl -s http://localhost:3000/tasks/真实UUID | jq '{status: .task.status, errorCode: .task.errorCode}'
```
...
1. **取消是运行时能力，不是 Prompt**：靠 `AbortSignal` 穿进 Agent 循环，不依赖模型「自觉停下」。
2. **cancelled ≠ failed**：前者是用户/超时中止；后者是工具/LLM 业务错误。
3. **进程内 Registry**：单 Node 进程用 Map 即可；多实例需共享协调（本仓库刻意不做）。
4. **协作式 vs 抢占式**：先保证步进边界可停；LLM SDK 传 `signal` 是后续增强。

#### 代码怎么读

| 顺序 | 文件 | 看什么 |
|------|------|--------|
| 1 | `apps/api/src/runtime/running-task-registry.ts` | taskId → AbortController |
| 2 | `apps/api/src/runtime/task-runner.ts` | register / timeout / cancelled 落库 |
| 3 | `apps/api/src/runtime/abort-utils.ts` | throwIfAborted / isTaskCancellation |
| 4 | `apps/api/src/agents/planner-agent.ts` | 步进边界 throwIfAborted |
| 5 | `apps/api/src/server.ts` | cancel 路由 + SSE close |
| 6 | `packages/api-contract/src/schemas.ts` | TaskStatus + CancelTaskResponse |

心智模型：`run` 登记 controller → cancel/超时/断开 abort → Planner `throwIfAborted` → TaskRunner 写 `cancelled`。
...
P3   E.4 新工具 + 安全治理                ← 已完成
P4   E.5 巩固周（读通 + eval 手练）       ← 已完成
P5   E.6 扩 eval + 改坏实验              ← 已完成
P5.5 E.6-B 新工具 list_dir              ← 已完成
