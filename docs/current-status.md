...
最后更新：2026-07-09（E.7-A 轻量 RAG search_docs + 学习计划）
...
**当前开发重点：** **E.7-A 轻量 RAG（关键词检索 `search_docs`）** 已交付；eval 基线 **22 条**。学习计划见 [`docs/backend-learning/lightweight-rag-plan.md`](backend-learning/lightweight-rag-plan.md)；改坏实验见 [`docs/backend-learning/eval-break-lab.md`](backend-learning/eval-break-lab.md)。
...
| | |
|--|--|
| **状态** | E.7-A 已完成；E.7-B（向量 + embedding）未开始 |
| **目标** | 自己串 RAG 最小闭环：切块 → 内存索引 → 检索 Tool → Planner 选用 → eval |
...
- Embedding API + Postgres/pgvector 持久化索引
- `pnpm run rag:index` 离线建索引
- 详见 `lightweight-rag-plan.md` §阶段 2
...
P6.5 E.7-B 向量 RAG（embedding）       ← 未开始
```
