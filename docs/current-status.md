...
**当前开发重点（后端优先）：** 见 [【E 节】](#e-后端优先路线当前采用) —— 当前 P3 是 **E.4 新工具 + 安全**。E.1–E.3 已完成（含 SSE + 最小前端流式 UI）。
...
**你现在最该做（P3）：** 实现 **E.4 新工具 + 安全治理** —— 新增实用工具 + eval 覆盖越权 case。
...
| **SSE 流式（Step 5 / E.3）** | **已完成（最小 UI）** —— chat 已接 `POST /agent/stream` |
...
| SSE `POST /agent/stream` | 已完成 | E.3 |
| 前端最小流式 chat | 部分完成 | E.3；Step 5 完整 UI 仍可选 |
...
| 验收项 | 状态 |
|--------|------|
| 后端 SSE `POST /agent/stream` | 已完成 |
| 前端流式消息 UI（token 逐段 + thinking） | 已完成 |
| 工具调用过程实时展示（debug 面板） | 部分完成 |
| AI SDK / 完整 Step 5 调试面板 | 未开始 |

**Step 5 总状态：部分完成**（E.3 最小联调已验收）
...
|--|--|
| **状态** | 已完成 |
| **目标** | 后端推送 `thinking` / `tool_start` / `tool_end` / `token` / `done` / `error`；前端最小流式 UI |
| **改动范围** | `POST /agent/stream`、`packages/api-contract/src/stream-events.ts`、`PlannerAgent` + `TaskRunner`、`apps/web` chat |
