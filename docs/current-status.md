...
最后更新：2026-07-08（E.6-B 新工具 list_dir + eval）
...
**当前开发重点：** **E.6-B 新工具 `list_dir`** 已交付；eval 基线 **20 条**。改坏实验见 [`docs/backend-learning/eval-break-lab.md`](backend-learning/eval-break-lab.md)。
...
**用例何时拆文件：** 当前单文件 20 条即可；到 **~30 条** 或难维护时按 [`docs/evals-and-replay.md` §用例组织策略](evals-and-replay.md#用例组织策略何时拆文件) 拆成 `smoke-tools` / `security` / `memory` 等，并改 `run-evals.ts` 合并 `cases/*.json`。
